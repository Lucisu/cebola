...
class CommonIssues extends NodeVisitorAbstract {
	...
	public function enterNode( Node $node ) {
		...
		if ( $node instanceof Node\Expr\FuncCall ) {
			if ( $node->name instanceof Node\Name ) {
				...
				$args  = $node->args;
				if ( in_array( $parts[0], $this->getWpFunctions() ) ) {
	 ...
	 * @return array
	 */
	private function getWpFunctions() {
		return array( 'update_option', 'update_user', 'wp_delete_user' );
	}
	...
	private function checkValueIsStringVarOrGlobal( $expr ) {
		if ( $expr->value instanceof Node\Scalar\String_ ) {
			...
			if ( in_array( $expr->value->var->name, array( '_GET', '_POST', '_REQUEST', '_SERVER' ) ) ) {
				return 2;
			} else {
				return 1;
			}
